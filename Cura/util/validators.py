	...
	def validate(self):
		...
		try:
			...
			if wallThickness <= nozzleSize * 0.5:
				return ERROR, 'Trying to print walls thinner than the half of your nozzle size, this will not produce anything usable'
			if wallThickness <= nozzleSize * 0.85:
				return WARNING, 'Trying to print walls thinner than the 0.8 * nozzle size. Small chance that this will produce usable results'
