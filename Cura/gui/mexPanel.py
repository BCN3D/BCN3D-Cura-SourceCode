...
class ProfileInfo(object):
    def __init__(self, filename):
        ...
        if cp.has_option('info', 'extruder_name'):
            self.extruder = cp.get('info', 'extruder_name')
...
class ProfileManager(object):
    ...
    def getExtruderNames(self):
        ret = []
        for profile in self._extruder_profiles:
            if profile.extruder_name not in ret:
    ...
    def isValidProfileOption(self, profile_name, extruder_name, material_name):
        return self._getProfileFor(profile_name, extruder_name, material_name) is not None

    def getSettingsFor(self, profile_name, extruder_name, material_name):
        settings = {}

        current_profile = self._getProfileFor(profile_name, extruder_name, material_name)
        cp = configparser.ConfigParser()
    ...
    def _getProfileFor(self, profile_name, extruder_name, material_name):
        if self._material_in_print_profile:
            for profile in self._print_profiles:
                if profile.name == profile_name and profile.material == material_name:
                    return profile
        else:
            for profile in self._print_profiles:
                if profile.name == profile_name and profile.extruder_name == extruder_name:
                    return profile
        return None
...
class MexPanel(wx.Panel):
    ...
    def __init__(self, parent, callback):
        ...
        for extruder_name in self._profile_manager.getExtruderNames():
            button = wx.RadioButton(printExtruderPanel, -1, extruder_name, style=wx.RB_GROUP if len(self._print_extruder_options) == 0 else 0)
            button.name = extruder_name
            self._print_extruder_options.append(button)
            if profile.getPreference('simpleModeExtruder') == extruder_name:
                button.SetValue(True)
    ...
    def _updateAvailableOptions(self):
        ...
        if not self._profile_manager.isValidProfileOption(profile_name, extruder_name, material_name):
            for button in self._print_profile_options:
                if self._profile_manager.isValidProfileOption(button.name, extruder_name, material_name):
                    button.SetValue(True)
                    profile_name = button.name
        if not self._profile_manager.isValidProfileOption(profile_name, extruder_name, material_name):
            for button in self._print_material_options:
                if self._profile_manager.isValidProfileOption(profile_name, extruder_name, button.name):
                    button.SetValue(True)
        ...
        if not self._profile_manager.isValidProfileOption(profile_name, extruder_name, material_name):
            for button in self._print_extruder_options:
                if self._profile_manager.isValidProfileOption(profile_name, extruder_name, button.name):
                    button.SetValue(True)
                    extruder_name = button.name
        if not self._profile_manager.isValidProfileOption(profile_name, extruder_name, material_name):
            for p_button in self._print_profile_options:
                for m_button in self._print_material_options:
                    for e_button in self._print_extruder_options:
                        if self._profile_manager.isValidProfileOption(p_button.name, m_button.name, e_button.name):
                            m_button.SetValue(True)
                            p_button.SetValue(True)
                            e_button.Setvalue(True)
                            profile_name = p_button.name
                            material_name = m_button.name
                            extruder_name = e_button.name


        for button in self._print_material_options:
            button.Enable(self._profile_manager.isValidProfileOption(profile_name, extruder_name, button.name))
        for button in self._print_extruder_options:
            button.Enable(self._profile_manager.isValidProfileOption(profile_name, extruder_name, button.name))
        for button in self._print_profile_options:
            button.Enable(self._profile_manager.isValidProfileOption(button.name, extruder_name, material_name))
    ...
    def getSettingOverrides(self):
        ...
        settings.update(self._profile_manager.getSettingsFor(profile_name, extruder_name, material_name))
